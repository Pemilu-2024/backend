...
public function inputSuara(Request $request)
{
    ...
    //     '*.tpsId' => 'numeric|required_with:*.jumlahSuara,*.kandidatId',
    //     '*.kandidatId' => 'numeric|required_with:*.jumlahSuara,*.tpsId',
    //     '*.bukti_suara' => 'required_without_all:jumlahSuara,tpsId,kandidatId|string'
    // ]);

    try {
        $idTps = '0';
        $buktiSuaraBase64 = null; // Inisialisasi variabel untuk menyimpan base64 terakhir
        ...
        // Setelah loop selesai, simpan base64 terakhir ke objek TPS jika ada
        if ($buktiSuaraBase64) {
            // Decode Base64 dan simpan sebagai file
            $buktiSuaraBinary = base64_decode($buktiSuaraBase64);
            $imageName = uniqid() . '.png'; // Nama file unik
            $path = storage_path('app/public/fileBukti/' . $imageName);
